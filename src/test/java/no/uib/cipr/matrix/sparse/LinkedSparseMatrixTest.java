...
public class LinkedSparseMatrixTest extends SparseMatrixTestAbstract {
    ...
    /**
     * Does a naive perf test against DenseMatrix, outputting CSV that we plot
     * in R.
     * <p/>
     * Generate n x n matrix with m entries, on left, and an n x n dense matrix
     * with m entries on the right, using the same population algo. Then we
     * multiply them and output into a dense matrix. We compare dense vs linked
     * sparse from the left, and also look at memory usage. We repeat for 10
     * different values of m (10,000 to 100,000), and in both cases vary m from
     * 1,000 to 10,000. This requires about 8GB heap to be on the safe side.
     * <p/>
        ...
        for (int r = 0; r < 10; r++) {
            for (int m = 10000; m <= 100000; m = m + 10000) {
                for (int n = 1000; n <= 10000; n = n + 1000) {
                    ...
                    String[] line = new String[]{Integer.toString(n),
                            Integer.toString(m), Long.toString(denseMem),
                            Long.toString(denseInitTime),
                            Long.toString(denseMultTime),
