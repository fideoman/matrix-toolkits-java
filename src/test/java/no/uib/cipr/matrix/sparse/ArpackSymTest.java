...
public class ArpackSymTest {
    ...
    public void testRandomEigensystem() throws NotConvergedException {
        for (int i = 100; i <= 500; i = i + 100) {
            ...
            for (Map.Entry<Double, DenseVectorSub> e : results.entrySet()) {
                ...
                if (!value)
                    fail("no matching eigenvalue for " + e.getKey() + " : "
                            + evd.keySet());
                if (!vector)
                    fail("no matching eigenvector for " + e.getKey() + " : "
                            + evd.keySet());
            }
        }
    }

    private Map<Double, DenseVector> evdSolve(UpperSymmDenseMatrix matrix)
    ...
    public static void main(String[] args) throws Exception {
        ...
        Matrix A = new LinkedSparseMatrix(new MatrixVectorReader(
                new FileReader(file)));
        Matrix AtA = A.transAmult(A,
                new LinkedSparseMatrix(A.numColumns(), A.numColumns()));
